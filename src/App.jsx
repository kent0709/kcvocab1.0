...
const App = () => {
        ...
        try {
          ...
          if (docSnap.exists()) {
            ...
            if (data.queue) {
              ...
              const initialQueue = Array.from({ length: data.cards.length }, (_, i) => i);
    ...
    if (match) {
      return (
        <div className="space-y-0.5">
          ...
          {match[2].trim() && <div className="text-[11px] font-medium text-indigo-500 leading-tight">{match[2].trim()}</div>}
  ...
  const handleTextSubmit = () => {
    ...
    const parsedCards = rows.map(row => {
      ...
      if (row.includes(',') || row.includes('，')) {
        ...
        if (firstSpace !== -1) {
          parts = [row.substring(0, firstSpace), row.substring(firstSpace + 1)];
        } else {
          parts = [row, ""];
        }
      }
    ...
    try {
      ...
      const newCards = parsedData.map(item => {
        if (isJapaneseMode) {
          ...
          let infoStr = `${item.reading ? item.reading + ' ' : ''}${item.meaning}`;
          if (item.derivatives) infoStr += ` 💡 [詞性變化]\n${item.derivatives}`;
          if (item.collocations) infoStr += ` 💡 [搭配詞]\n${item.collocations}`;
  ...
  const getRating = () => {
    ...
    if (score === 100) return { score, text: "完美全對", color: "text-yellow-500", emoji: "🏆" };
    if (score >= 90) return { score, text: "優秀九成", color: "text-green-500", emoji: "🌟" };
    if (score >= 80) return { score, text: "穩健八成", color: "text-blue-500", emoji: "👍" };
    if (score >= 60) return { score, text: "及格", color: "text-orange-500", emoji: "👌" };
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50 p-6">
        <div className="bg-white p-8 rounded-[2rem] shadow-xl max-w-md w-full text-center border border-slate-100">
          ...
          <p className="text-slate-500 mb-6 text-sm font-medium">貼上純日文單字，讓 AI 為你自動補齊完整解釋與例句</p>
          ...
          <div className="flex flex-col gap-3 mb-6">
            ...
            >
              {isGeneratingCards ? <Loader2 size={18} className="animate-spin" /> : <Star size={18} className="text-yellow-300" />}
              {isGeneratingCards ? 'AI 正在分析建立中...' : '✨ AI 自動生成完整單字卡'}
            </button>
          ...
          <div className="relative flex py-2 items-center mb-5">
            ...
            <span className="flex-shrink-0 mx-4 text-slate-400 text-xs font-bold uppercase tracking-wider">或使用檔案</span>
            <div className="flex-grow border-t border-slate-100"></div>
          </div>
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50 p-6">
        <div className="bg-white p-10 rounded-[2rem] shadow-2xl max-w-lg w-full text-center border border-slate-100">
          <div className="text-6xl mb-4 animate-bounce drop-shadow-md">{rating.emoji}</div>
          <h1 className="text-3xl font-black text-slate-800 mb-2">完成本日練習！</h1>
          <div className={`text-2xl font-black ${rating.color} mb-2`}>{rating.score} 分 - {rating.text}</div>
          <p className="text-slate-500 font-medium mb-8">持之以恆，語言就是你的肌肉記憶。</p>
          ...
          <div className="grid grid-cols-4 gap-2 my-8">
            <div className="bg-red-50 p-3 rounded-2xl border border-red-100 flex flex-col items-center justify-center">
              ...
              <p className="text-red-500 text-[10px] font-bold uppercase tracking-widest">Again</p>
            </div>
            <div className="bg-orange-50 p-3 rounded-2xl border border-orange-100 flex flex-col items-center justify-center">
              ...
              <p className="text-orange-500 text-[10px] font-bold uppercase tracking-widest">Hard</p>
            </div>
            <div className="bg-blue-50 p-3 rounded-2xl border border-blue-100 flex flex-col items-center justify-center">
              ...
              <p className="text-blue-500 text-[10px] font-bold uppercase tracking-widest">Good</p>
            </div>
            <div className="bg-green-50 p-3 rounded-2xl border border-green-100 flex flex-col items-center justify-center">
              ...
              <p className="text-green-500 text-[10px] font-bold uppercase tracking-widest">Easy</p>
            </div>
          </div>

          <button 
            onClick={async () => {
              const resetQueue = Array.from({ length: cards.length }, (_, i) => i);
              ...
              if (currentDeckId && user) {
                try {
                  await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'decks', currentDeckId), {
                    queue: resetQueue, history: resetHistory
                  });
                } catch (e) {}
              }
            }}
      ...
      <div className="w-full max-w-md mb-4 space-y-3">
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-2 text-slate-700 font-black text-lg bg-white px-4 py-1.5 rounded-full shadow-sm border border-slate-100">
            ...
            <span>{totalInitial - queue.length} <span className="text-slate-400 text-sm font-medium">/ {totalInitial}</span></span>
          ...
          {shareUrl || currentDeckId ? (
            <button 
              ...
              className={`flex items-center gap-1.5 px-4 py-2 rounded-full text-xs font-bold transition-all shadow-sm ${copySuccess ? 'bg-green-500 text-white shadow-green-200' : 'bg-white text-indigo-600 border border-indigo-100 hover:bg-indigo-50 shadow-indigo-100'}`}
            >
        ...
        <div className={`relative w-full h-full transition-all duration-500 transform-style-3d ${isFlipped ? 'rotate-y-180' : ''}`}>
          ...
          <div className="absolute inset-0 backface-hidden rotate-y-180 bg-white rounded-[2rem] shadow-xl shadow-slate-200/50 flex flex-col p-4 sm:p-5 overflow-hidden border border-slate-100">
            ...
            <div className="w-full h-[100px] sm:h-28 bg-slate-100 rounded-xl mb-3 overflow-hidden relative flex items-center justify-center shadow-inner shrink-0">
              {imageUrls[currentCard.word] ? (
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-400 z-0 bg-slate-50">
                  ...
                  <span className="text-[10px] font-black tracking-widest uppercase text-indigo-400">Generating...</span>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-5 gap-1 sm:gap-2 mt-2 pt-2 border-t border-slate-100 shrink-0">
              <button onClick={(e) => { e.stopPropagation(); handleSrsAction('again'); }} className="flex flex-col items-center gap-1.5 p-1 hover:bg-slate-50 hover:scale-105 active:scale-95 rounded-xl transition-all">
                ...
                <span className="text-[9px] sm:text-[11px] font-black uppercase tracking-wider text-red-500">Again</span>
              </button>

              <button onClick={(e) => { e.stopPropagation(); handleSrsAction('hard'); }} className="flex flex-col items-center gap-1.5 p-1 hover:bg-slate-50 hover:scale-105 active:scale-95 rounded-xl transition-all">
                ...
                <span className="text-[9px] sm:text-[11px] font-black uppercase tracking-wider text-orange-500">Hard</span>
              </button>

              <button onClick={(e) => { e.stopPropagation(); speak(getSpeakableText(currentCard)); }} className="flex flex-col items-center justify-start gap-1 p-1 hover:scale-105 active:scale-95 transition-all group">
                ...
                <span className="text-[10px] sm:text-[11px] font-black uppercase tracking-wider text-indigo-600 transform -translate-y-1">Listen</span>
              </button>

              <button onClick={(e) => { e.stopPropagation(); handleSrsAction('good'); }} className="flex flex-col items-center gap-1.5 p-1 hover:bg-slate-50 hover:scale-105 active:scale-95 rounded-xl transition-all">
                ...
                <span className="text-[9px] sm:text-[11px] font-black uppercase tracking-wider text-blue-500">Good</span>
              </button>

              <button onClick={(e) => { e.stopPropagation(); handleSrsAction('easy'); }} className="flex flex-col items-center gap-1.5 p-1 hover:bg-slate-50 hover:scale-105 active:scale-95 rounded-xl transition-all">
                ...
                <span className="text-[9px] sm:text-[11px] font-black uppercase tracking-wider text-green-500">Easy</span>
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4" onClick={() => setShowRatingModal(false)}>
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl relative" onClick={e => e.stopPropagation()}>
            ...
            <div className="bg-slate-50 p-4 rounded-2xl mb-6 border border-slate-100">
              ...
              <div className="text-2xl font-black text-indigo-600">{Math.round(((totalInitial - queue.length) / totalInitial) * 100)}%</div>
            </div>
            
            <button onClick={() => setShowRatingModal(false)} className="w-full py-3.5 bg-slate-800 hover:bg-slate-900 text-white rounded-xl font-bold transition-all shadow-lg">
              繼續練習
            </button>
          </div>
        </div>
